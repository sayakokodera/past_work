...
def spreadingfunctionmat(spreadingfunctionBarkdB, alpha, nfilts, tonalityidx):
    ...
    r""" NEW 2020.01.30
    We introduced Ofz (= distance to masking threshold) to reflect the tonality index.
    See slides psc model p.10 and/or the lecture book p.136 for more detail.
    """
    spreadingfuncmatrix = np.zeros((nfilts, nfilts))
    for k in range(nfilts):
        if tonalityidx is None:
            Ofz = 23.5  # default value from the book
        else:
            Ofz = tonalityidx * (14.5 + k) + (1 - tonalityidx) * 5.5
        curr_spreadingfunctionBarkdB = spreadingfunctionBarkdB - Ofz
        spreadingfunctionBarkVoltage = 10.0 ** (curr_spreadingfunctionBarkdB / 20.0 * alpha)
...
def maskingThresholdBark(mXbark, spreadingfuncmatrix, alpha, fs, nfilts):
    ...
    mTbark = np.maximum((mTbark, 10.0 ** ((LTQ - 60) / 20)), 0)
    return mTbark
