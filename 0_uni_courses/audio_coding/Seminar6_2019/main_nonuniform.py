...
sRate, wavArray = wavOperations().getWavParameters("Audio Samples-20200115/rockyou_16.wav")
DFTResolution = 1024
barkResolution = 48
...
exponent_list = [0.6, 1, 0.75]
gain = {}
#quality = 60 #Higher values increase the audio quality, lower values decrease the bit rate.

for exponent in exponent_list:

    yq, mTbarkquant = f.analysisAndQuantisation(sampleFrequency, ys,AnalysisFilterResult, DFTResolution, exponent = exponent)

    # Decoding the encoded.bin
    xhat, Xhat = f.decoder('encoded.bin',sampleFrequency, exponent = exponent)
    ...
    gain[exponent] = {'size' : os.stat("encoded.bin").st_size, 'SNR': SNR}
...
plt.plotGainandSNR(gain, 'tonality index')
