...
#Fmat=np.zeros((N,N))
#Fmat[0:int(int(N/2)),0:int(int(N/2))]=np.fliplr(np.diag(fcoeff[0:int(int(N/2))]))
#Fmat[int(int(N/2)):N,0:int(int(N/2))]=np.diag(fcoeff[int(int(N/2)):N])
#Fmat[0:int(int(N/2)),int(int(N/2)):N]=np.diag(fcoeff[N:(N+int(int(N/2)))])
#Fmat[int(int(N/2)):N,int(N/2):N]=-np.fliplr(np.diag(fcoeff[(N+int(N/2)):(2*N)]))
...
if __name__ == '__main__':
     ...
     H2invmatrix.z=np.zeros(N)
     Ginvmatrix.z[0]=np.zeros(int(N/2))
     Ginvmatrix.z[1]=np.zeros(int(N/2))
