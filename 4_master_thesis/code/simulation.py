# -*- coding: utf-8 -*-
"""
Monte Carlo simulation for reconstructing interpolated MUSE data
"""
...
def ss_randomwalk(Ns, Nx, Ny):
    N_walk = 0
    
    while N_walk < Ns:
        walk = batch_random_walk2D(3*Ns, Nx, Ny)
...
def spatial_subsampling(Ns):
    print('Spatial subsampling!')
    N_smp = 0
    while N_smp < Ns:
        # Set 1
        #p_smp1 = ss_randomwalk(Ns = int(0.2* Ns), Nx = np.random.randint(17, 20), Ny = np.random.randint(12, 16))
        ...
        # Set2
        #p_smp2 = ss_randomwalk(Ns = int(0.2* Ns), Nx = np.random.randint(17, 20), Ny = np.random.randint(12, 16))
        p_smp2 = batch_subsampling(Nx = np.random.randint(17, 20), N_scan = int(0.2* Ns), Ny = np.random.randint(12, 16))
        p_smp2[:, 0] += np.random.randint(25, 30)
...
def resampling_krig(p_smp_roi, p_resmp_rnd, rmse, rth, Nx, Ny, Ns_resmp, combine = True):
    ...
    if Ns_krig > Ns_resmp:
        ...
        indices = np.random.choice(np.arange(Ns_krig), Ns_resmp, replace = False)
...
buffer_setNo = 40
#if buffer_setNo != 0:
#    raise ValueError('buffer_setNo is not zero!!!!!')
# data saving
tdf = DateTimeFormatter()
today = '210625'#tdf.get_date_str()

### Spatial subsampling ###
coverage_all = 0.01* np.flip(np.array([5, 6, 7, 8, 9, 10, 12, 14, 15, 17])) 
...
def interpolate_and_saft(setNo_vec):
    
    ##############################
    ######## Function ########
    def batch_interpolation(p_start, scan_map, without_fk = False):
        #print('=======================================')
        ...
        # No samples in the batch -> ignore
        if p_smp.shape[0] == 0:
        ...
        # Case: (i) samples < 5 or (ii) min(lags) > maxlag -> no interpolation 
            ...
            if without_fk == True:
                #  Dummy
                Ahat_DNN = np.zeros(Ahat_idw.shape)
                rmse_FKDNN = np.nan* np.ones((N_batch, N_batch))
   ...
   #=====================================#
    ...
    ######## Start ########
    #=====================================#
    # Check the coverage & setNo
    coverage = coverage_all[int(setNo_vec[0] / N_realization)]
    setNo = setNo_vec[0] % N_realization
    
    # File name setting
    path = 'npy_data/simulations/{}_{}%/setNo_{}'.format(today, int(100*coverage), int(setNo + buffer_setNo))
    
    print('============================================')
    print('New scan map! coverage = {}, setNo.{}'.format(coverage, setNo + buffer_setNo))
    start_batch = time.time()
    
    #=====================================#
    ### (1) Spatial subsampling ###
    #=====================================#
    # Number of initial scans
    Ns = int(coverage* Nx* Ny) 
    
    # Selecting the positions
    p_smp_roi = spatial_subsampling(Ns)
    save_data(p_smp_roi, path, 'p_smp.npy')
    
#    # Load the sampling positions
#    print('Loading the scan positions!')
#    p_smp_roi = np.load('{}/p_smp.npy'.format(path))
    
    # Scan map
    scan_map = make_scanmap(p_smp_roi, Nx, Ny)
    ...
    #=====================================#
    ### (2) Batch-wise interpolation ###
    #=====================================#
    print('Start batch-wise interpolation!')
    # Base
    A_idw = np.zeros(A_roi.shape)
    A_fkdnn = np.zeros(A_roi.shape)
    rmse_fk = np.zeros((Nx, Ny))
    
    for batchNo, p in enumerate(p_batch_all):
        # Interpolate
        arr_idw, arr_fkdnn, e_fkdnn = batch_interpolation(p, scan_map)
        ...
        rmse_fk = batch_moving_window_assignment(e_fkdnn, rmse_fk[np.newaxis, ...], x_start, y_start)[0, ...]

    print('Interpolation finished!')
    del arr_idw, arr_fkdnn, e_fkdnn
    ...
    # (b) "Optimal"
    p_resmp_krig, Ns_krig = resampling_krig(p_smp_roi, p_resmp_rnd, rmse_fk, rth, Nx, Ny, Ns)
    ...
    for batchNo, p in enumerate(p_batch_all):
        # Interpolation
        # (a) Random resampling
        arr_idw, _, _ = batch_interpolation(p, scanmap_rernd, without_fk = True)
        # (b) "Optimal" resampling
        _, arr_fkdnn, e_fkdnn = batch_interpolation(p, scanmap_rekrig)
    ...
    # Save
    # RMSE
    save_data(rmse_fk, path, 'rmse_FK.npy')
    save_data(rmse_re_fk, path, 'rmse_re_FK.npy')
    del rmse_fk, rmse_re_fk
    
    # Resampling positions
    save_data(p_resmp_rnd[Ns:, :], path, 'p_resmp_rnd.npy')
    save_data(p_resmp_krig[Ns:, :], path, 'p_resmp_krig.npy') # Ns_krig
    save_data(Ns_krig, path, 'Ns_krig.npy') 
    del p_resmp_rnd, p_resmp_krig  
    ...
    save_data(R_smp, path, 'R_smp.npy')
    del R_smp, A_smp
    
    # IDW
    R_idw = calc_reco(A_idw, saft_engine)
    save_data(R_idw, path, 'R_idw.npy')
    del R_idw, A_idw
    
    # Random resampling
    R_re_idw = calc_reco(A_re_idw, saft_engine)
    save_data(R_re_idw, path, 'R_re_idw.npy')
    del R_re_idw, A_re_idw
    
    # FK DNN
    R_fk = calc_reco(A_fkdnn, saft_engine)
    save_data(R_fk, path, 'R_fk.npy')
    del R_fk, A_fkdnn
    
    # "Optimal" resampling
    R_re_fk = calc_reco(A_re_fk, saft_engine)
    save_data(R_re_fk, path, 'R_re_fk.npy')
    del R_re_fk, A_re_fk
     
    print('Recos finised!')
    display_time(round(time.time() - start_batch, 5))
...
np.apply_along_axis(interpolate_and_saft, 0, np.array([np.arange(N_coverage* N_realization)]))
