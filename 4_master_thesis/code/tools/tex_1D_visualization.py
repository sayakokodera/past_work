...
def generate_coordinates_for_addplot(x_values_input, file_name, x_y_reverse, colors, linestyle, *args):
    ...
    parameters : 
        ...
        file_name 
            file name for the generated tex file (!!! .tex should be included !!)
        x_y_reverse
    ...
    for idx in range(len(y_values)):
        coord = ''
        for row, curr_x in enumerate(x_values):
            str_x = str(curr_x)#str(round(curr_x, 2))
            str_y = str(y_values[idx][row])#str(round(y_values[which_y][x_values.index(curr_x)], 2))
            #print('No.{}: (str_x, str_y) = ({}, {})'.format(row, str_x, str_y))            
            # str_x, str_y ----> str_coord
            if x_y_reverse == True :
                coord += r""" ({}, {})
                         """.format(str_y, str_x)
