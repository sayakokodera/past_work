...
class ThermoDataPreparation():
    ...
    def load(self, pos, *args, **kwargs):
        data_tens, self.fileNo = self.reader.load_data(
            pos=pos, 
            y_offset=self.y_offset, xmin=self.xmin, xmax=self.xmax, 
            dtype=self.dtype, width=self.width, height=self.height,
            ret_fileNo=True
        )
        return data_tens

    def process(self, arr, *args, **kwargs):
        # Center the data
        ...
        # FFT phase reco
        reco = proc.reco_fft_phase(arr_trimmed)[..., 1] # only fbin=1 is relevant
        # Zero-mean
        mean = reco.mean()
        reco = reco - mean
        return reco
    ...
    def get_reco(self, pos, *args, **kwargs):
        # (1) Load
        data_tens = self.load(pos)
        # (2) Process
        reco = self.process(data_tens)
        return reco
